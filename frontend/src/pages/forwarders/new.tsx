...
export default function (): JSX.Element {
  ...
  const { data: messages, error: messagesError } = useSWR(fromChat ? `/chats/${fromChat}/${number}` : undefined, fetcher)
  ...
  const onSubmit = async (ev: any) => {
    ...
    try {
      const toUrl = new URL(import.meta.env.VITE_BACKEND_URL)
      toUrl.searchParams.set("h-Access-Control-Allow-Origin", "*")
      toUrl.searchParams.set("url", `http://129.148.60.94/forwarders`)
      await fetch(toUrl, {
        method: "POST",
        headers: {
